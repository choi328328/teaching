    ...
    def calculate_frax(
        ...
        with contextlib.suppress(Exception):
            ...
            self.driver.find_element(
                By.ID, "ctl00_ContentPlaceHolder1_bmd_input"
            ).send_keys(Keys.ENTER)

        self.driver.find_element(By.ID, "ctl00_ContentPlaceHolder1_bmd_input").clear()
        self.driver.find_element(
            By.ID, "ctl00_ContentPlaceHolder1_bmd_input"
        ).send_keys(femoral_value)
        time.sleep(0.5)
        self.driver.find_element(
            By.ID, "ctl00_ContentPlaceHolder1_btnCalculate"
        ).click()
        with contextlib.suppress(Exception):
            self.driver.switch_to.alert
        ...
        osteo = self.driver.find_element(
            By.CSS_SELECTOR, ".result-box:nth-child(2) .result-score"
        ).text
        fracture = self.driver.find_element(
            By.CSS_SELECTOR, ".result-box:nth-child(3) .result-score"
        ).text
        return float(pat_id), float(osteo), float(fracture)
